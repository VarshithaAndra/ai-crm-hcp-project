...
The database layer is implemented using **SQLAlchemy**, which maps Python objects to relational database tables.

#### Responsibilities:

* Define schema using ORM models
* Manage database sessions and connections
* Insert AI-processed interaction records
* Retrieve interaction history for UI display

---

### Data Flow Integration

The database is tightly integrated with the AI pipeline:

1. User provides input (form or chat)
2. LangGraph processes input via LLM
3. Structured JSON output is generated
4. FastAPI validates the response
5. Data is stored in SQL database
6. Records are retrieved and displayed in UI
...
| Reason             | Explanation                                  |
| ------------------ | -------------------------------------------- |
| Structured Storage | Ensures consistent schema for CRM data       |
| Query Support      | Enables filtering (by HCP, date, sentiment)  |
| Scalability        | Suitable for enterprise-level applications   |
| Integration        | Works seamlessly with FastAPI via SQLAlchemy |

---

### Key Contribution

The database layer was designed to bridge the gap between:

* Unstructured conversational input
* Structured enterprise CRM storage

This ensures that AI-generated insights are not only processed but also persistently stored and reusable for analytics and reporting.
